...
def generate_formatted_pdf(html_content, chat_id, is_full_chat=False):
    ...
    try:
        ...
        filename = f"Conversation_{chat_id}_{'full' if is_full_chat else 'partial'}_{timestamp}.pdf"
...
def ask_question():
    ...
    try:
        ...
        return jsonify({
            'answer': answer.strip(),  # Ensure the answer is plain text
            'chat_id': chat_id
        })
    except Exception as e:
    ...
    try:
        ...
        if filepath and os.path.exists(filepath):
            return send_file(
                filepath,
                as_attachment=True,
                download_name=f"Conversation_{chat_id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
            )
